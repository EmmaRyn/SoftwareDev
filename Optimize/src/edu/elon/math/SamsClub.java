/*
 * SamsClub.java 1.0 August 20, 2014
 *
 ...
 * 
 */
public class SamsClub extends Function implements Remote {
  ...
  public Double evaluate() throws RemoteException {
    double x;
      x = getInputValues().get(0).doubleValue();
      double y = getInputValues().get(1).doubleValue();
      double cost =
                      60.0 / (1 + Math.pow(x + 1, 2) + Math.pow(y - 3, 2)) + 20.0
                      / (1 + Math.pow(x - 1, 2) + Math.pow(y - 3, 2)) + 30.0
                      / (1 + Math.pow(x, 2) + Math.pow(y + 4, 2));
